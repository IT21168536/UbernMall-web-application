...
import animationData from '../components/images/Animation - 1710827989253';
...
function Login(){
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if(data.email === "Supplier@gmail.com" && data.password === "admin"){
                toast.success("Admin Login Success!");
                window.location = "/Dashboad";
            }
            else {
                const url = "http://localhost:5000/api/supplierauth";
                    ...
                    <div className="container" style={{ display: "flex" }}>
                        ...
                        <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                                ...
                                Supplier Sign in to your account
                                </h1>
                                <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                                    ...
                                    <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                        Don’t have an account yet? <Link to={"/suppliereg"} className="font-medium text-primary-600 hover:underline dark:text-primary-500">Sign Up</Link>
                                    </p>
                                </form>
                            </div>
                        </div>
