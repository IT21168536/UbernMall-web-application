...
import animationData from '../components/images/Animation - 1710833731279.json';
...
export default function AddEmployee(){
  ...
  function handleImage(e) {
   ...
   function sendData(e){
    ...
    axios.post("http://localhost:5000/employees/add", formData).then(()=>{
    ...
    return(
      <div>
        ...
        <form onSubmit={sendData} class="max-w-sm mx-auto">
           <div class="mb-5">
    <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900 white:text-dark" >Employee ID :</label>
    <input type="text" id="base-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value={ID}  aria-describedby="emailHelp" placeholder="Enter Employee Name"
    />

          {errors&&ID.length<=0?<label className="validation-label">ID cannot be empty</label>:""}
  </div>
        <div className="mb-5">
        <label for="base-input" class="block mb-2 text-sm font-medium text-gray-900 white:text-dark">Employee Name :</label>
...
{errors&&name.length<=0?<label className="validation-label">Name cannot be empty</label>:""}
  ...
  <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"id="designation" value={designation} onChange={(e) => setDesignation(e.target.value)}>
    ...
    <option value="Security service "></option>
    <option value="Waiter">Waiter</option>
    <option value="Reseptionist">Receptionist</option>
    <option value="Cleaner">Cleaner</option>
    <option value="Emergency Procedures">Driver</option>
  </select>
